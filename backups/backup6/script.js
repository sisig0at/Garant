(async function() {
    ...
    try {
        ...
        if (session && session.user && session.user.email) {
            sessionCheck = session.user.email.replace(/@vg\.local$/, '');
        }
    } catch(e) {}
        ...
        if (event === 'SIGNED_OUT') {
            ...
            if (u && !u.banned) {
                ...
                if (dealId && !isNaN(parseInt(dealId))) {
                    try { loadSingleDealPage(parseInt(dealId)); } catch(e) {}
                }
            }
        }
    });
    ...
    function startLiveFeed() {
        ...
        function addFeedEntry(html) {
            lastDealsFeedArray.unshift(html);
        ...
        function update() {
            var id1 = Math.floor(Math.random() * 900000) + 100000;
            var id2 = Math.floor(Math.random() * 900000) + 100000;
            var amount = Math.floor(Math.random() * 50000) + 1000;
            var texts = [
                truncateId('user_' + id1) + ' завершил сделку на ' + amount + ' ₽',
                'Новая сделка #' + (Math.floor(Math.random() * 900) + 1000) + ' на ' + (Math.floor(Math.random() * 30000) + 5000) + ' ₽',
                'Выплачено ' + (Math.floor(Math.random() * 80000) + 5000) + ' ₽ пользователю ' + truncateId('user_' + id2),
                'Диспут #' + (Math.floor(Math.random() * 900) + 1000) + ' решён',
                truncateId('user_' + id1) + ' повысил уровень доверия до ' + (Math.floor(Math.random() * 41) + 60) + '%'
            ];
            var text = texts[Math.floor(Math.random() * texts.length)];
            addFeedEntry(text + ' — ' + new Date().toLocaleTimeString());
            updateSystemStats({
                total_deals: (systemStats.total_deals || 0) + 1,
                total_turnover: (systemStats.total_turnover || 0) + amount
            }).then(function() { updateGlobalStats(); });
            setTimeout(update, 120000 + Math.random() * 180000);
        }
        update();
    }
    ...
    document.addEventListener('click', function(e) {
        ...
        if (dealItem && !target.closest('button')) {
            window.location.href = '?deal=' + dealItem.dataset.id;
            return;
        }
    ...
    async function handleSingleConfirm() {
        ...
        // Add to live feed
        lastDealsFeedArray.unshift(deal.seller + ' завершил сделку на ' + (deal.amount || 0) + ' ₽ с ' + deal.buyer + ' — ' + new Date().toLocaleTimeString());
        if (lastDealsFeedArray.length > 6) lastDealsFeedArray.pop();
        var feedDiv = document.getElementById('liveDealsFeed');
        if (feedDiv) {
            feedDiv.innerHTML = lastDealsFeedArray.map(function(t) {
    ...
    async function init() {
        ...
        await loadAllData();
        if (sessionCheck) {
            var u = findUserByLogin(sessionCheck);
            if (u && !u.banned) {
                currentUser = u;
            }
        }
