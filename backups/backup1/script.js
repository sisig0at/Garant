(function() {
    ...
    async function renderProfile() {
        ...
        if (container) {
            container.innerHTML =
                ...
                '<p><i class="fas fa-fingerprint"></i> <strong>ID аккаунта:</strong> #' + currentUser.id + '</p>';
        }
    ...
    async function renderAdminPanel() {
        ...
        if (userListDiv) {
            userListDiv.innerHTML = users.map(function(u) {
    ...
    function updateGlobalStats() {
        ...
        if (completedSpan) completedSpan.innerText = deals.filter(function(d) { return d.status === 'completed'; }).length;
    ...
    async function loginUser(login, pass) {
        if (login === 'violet_admin' && pass === 'admin2025') {
            ...
            if (!admin) {
                ...
                let saved = await upsertUser(newAdmin);
    ...
    function showPage(pageId) {
        ...
        if (pageId === 'adminPage' && currentUser && currentUser.role === 'admin') renderAdminPanel();
    ...
    function startLiveFeed() {
        ...
        function update() {
            var usrs = ['user_' + (Math.floor(Math.random() * 9000) + 1000), 'user_' + (Math.floor(Math.random() * 9000) + 1000)];
            var texts = [
                usrs[0] + ' завершил сделку на ' + (Math.floor(Math.random() * 50000) + 1000) + ' ₽',
                'Новая сделка #' + (Math.floor(Math.random() * 900) + 1000) + ' на ' + (Math.floor(Math.random() * 30000) + 5000) + ' ₽',
                'Выплачено ' + (Math.floor(Math.random() * 80000) + 5000) + ' ₽ пользователю ' + usrs[1],
                'Диспут #' + (Math.floor(Math.random() * 900) + 1000) + ' решён',
                usrs[0] + ' повысил уровень доверия до ' + (Math.floor(Math.random() * 41) + 60) + '%'
            ];
            var text = texts[Math.floor(Math.random() * texts.length)];
            lastDealsFeedArray.unshift(text + ' — ' + new Date().toLocaleTimeString());
            ...
            setTimeout(update, 45000 + Math.random() * 15000);
        }
        update();
    }
    ...
    function initFaq() {
        document.querySelectorAll('.faq-item').forEach(function(item) {
            ...
            if (q) {
                ...
                var handler = function() { item.classList.toggle('active'); };
    ...
    document.addEventListener('click', function(e) {
        ...
        if (target.closest('#singleDealChatSendBtn')) {
            var dealId = new URLSearchParams(window.location.search).get('deal');
            if (dealId) {
                var text = document.getElementById('singleDealChatInput').value.trim();
                if (text) {
                    sendSingleDealMessage(parseInt(dealId), text);
                }
        ...
        if (dealItem && !target.closest('button')) {
            window.open('?deal=' + dealItem.dataset.id, '_blank');
            return;
        }
    ...
    async function handleCreateDeal() {
        ...
        if (!item || isNaN(amount) || amount < 100 || !buyer) { showToast('Заполните все поля'); return; }
    ...
    async function fakePay() {
        ...
        if (deal && deal.status === 'waiting_payment' && currentUser && currentUser.login === deal.buyer) {
            ...
            await insertDealMessage(deal.id, sysMsg);
            if (window.location.search.includes('deal=' + deal.id)) loadSingleDealPage(deal.id);
            updateGlobalStats();
            document.getElementById('paymentModal').style.display = 'none';
        } else {
    ...
    function handleAdminAddBalance() {
    ...
    function handleAdminSetBalance() {
        var login = document.getElementById('adminLogin').value.trim();
        var amt = parseInt(document.getElementById('adminAmount').value);
        var u = users.find(function(x) { return x.login === login; });
        if (u && !isNaN(amt)) {
    ...
    function handleAdminBan() {
        ...
        if (u) {
            ...
            upsertUser(u).then(function() {
                ...
                renderAdminPanel();
                showToast(login + ' ' + (u.banned ? 'забанен' : 'разбанен'));
            });
        }
    }
    ...
    function handleSetCompleted() {
        ...
        if (!isNaN(newCompleted) && newCompleted >= 0) {
            var completedSpan = document.getElementById('completedCount');
            if (completedSpan) completedSpan.innerText = newCompleted;
            showToast('Отображаемое количество завершённых сделок: ' + newCompleted);
    ...
    async function init() {
        ...
        if (dealId && !isNaN(parseInt(dealId))) {
            ...
            document.getElementById('singleDealPage').style.display = 'block';
            loadSingleDealPage(parseInt(dealId));
        } else {
            document.getElementById('mainContent').style.display = 'block';
            document.getElementById('singleDealPage').style.display = 'none';
            showPage('homePage');
        }
